...
#include "details/server_socket.hpp"
#include "details/progress_timer.hpp"

namespace pmq {

server_socket::server_socket(const service_info& sinfo, const std::vector<host_info>& hosts, boost::shared_ptr<server_context> context) :
	service_info_(sinfo),
	hosts_(hosts),
	context_(context),
...
server_socket::connect() {
	zmq_control_socket_.reset(new zmq::socket_t(*(context_->zmq_context_), ZMQ_PAIR));
...
server_socket::process_message_queue() {
	...
	zmq_socket_.reset(new zmq::socket_t(*(context_->zmq_context_), ZMQ_DEALER));
	
	std::auto_ptr<zmq::socket_t> zmq_control_socket2;
	zmq_control_socket2.reset(new zmq::socket_t(*(context_->zmq_context_), ZMQ_PAIR));
	...
	try {
		for (size_t i = 0; i < hosts_.size(); ++i) {
			...
			int timeout = 0;
			zmq_socket_->setsockopt(ZMQ_LINGER, &timeout, sizeof(timeout));
			zmq_socket_->connect(connection_str.c_str());
		}
	}
	...
	while (is_running_) {
		...
		poll_items[0].socket = *zmq_socket_;
		poll_items[0].fd = 0;
		...
		if ((ZMQ_POLLIN & poll_items[0].revents) == ZMQ_POLLIN) {
			...
			while (true) {
				...
				// receive reply
				try {
					if (!zmq_socket_->recv(&reply, ZMQ_NOBLOCK)) {
						break;
					}
