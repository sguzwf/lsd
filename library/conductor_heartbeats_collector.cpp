...
#include "details/conductor_heartbeats_collector.hpp"
#include "details/helpers.hpp"
#include "details/structs.hpp"

namespace pmq {

conductor_heartbeats_collector::conductor_heartbeats_collector(const std::string& mip, unsigned short mport) :
	multicast_ip_(mip),
...
conductor_heartbeats_collector::~conductor_heartbeats_collector() {
	is_stopping_ = true;
...
void
conductor_heartbeats_collector::collect_heartbeat(std::string heartbeat) {	
...
void
conductor_heartbeats_collector::output_clients_info() {
	boost::mutex mutex;
	boost::mutex::scoped_lock lock(mutex);
...
void
conductor_heartbeats_collector::set_logger(boost::shared_ptr<base_logger> logger) {
	logger_ = logger;
}

void
conductor_heartbeats_collector::clean_up_dead_hosts() {	
...
void
conductor_heartbeats_collector::refresh_hostname() {
	get_hostname_and_ip(hostname_, ip_);
}

std::multimap<std::string, host_heartbeat>
conductor_heartbeats_collector::get_all_hosts() const {
	boost::mutex mutex;
	boost::mutex::scoped_lock lock(mutex);
...
void
conductor_heartbeats_collector::set_callback(boost::function<void(const std::string&)> f) {
	callback_ = f;
}

std::vector<host_info>
conductor_heartbeats_collector::get_hosts_by_group(const std::string& group) const {
	boost::mutex mutex;
	boost::mutex::scoped_lock lock(mutex);
...
void	
conductor_heartbeats_collector::run() {
	refresh_hostname();
	refresher_.reset(new refresher(boost::bind(&conductor_heartbeats_collector::clean_up_dead_hosts, this), HEARTBEAT_INTERVAL));
	
	mcast_server_.reset(new multicast_server(boost::bind(&conductor_heartbeats_collector::collect_heartbeat, this, _1)));
